...
class SunSpider(CrawlSpider):
    ...
    # for url in link_detail:
    #     url = 'https://wz.sun0769.com/' + url
    #     print(url)
    print(link)
    rules = (
        #规则解析器：将链接提取器提取到的链接进行指定规则（callback）的解析操作
        Rule(link, callback='parse_item', follow=False),
        #follow=True:可以将链接提取器继续作用到连接提取器提取到的连接，所对应的页面中
        Rule(link_detail,callback='parse_detail')

    )

    #political/politics/index?id=515484
    #political/politics/index?id=515480

    #解析新闻编号和新闻的标题
    #如下两个解析方法中是不可以实现请求传参！
    #无法将两个解析方法解析的数据存储到同一个item中，可以将此存储到两个item
